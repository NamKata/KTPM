...
class User
{
    ...
    public static function CapnhapSoluotTao($IdUser,$Soluot)
    {
        $db = new Db();
        $sql = " UPDATE `user` SET `SoluotTao`= `SoluotTao`+'$Soluot' WHERE `IdUser` ='$IdUser'";
        $result = $db->query_execute($sql);
        return $result;
    }
     public static function TruSoLuotTaoTruyen($IdUser,$Soluot)
    {
        $db = new Db();
        $sql = " UPDATE `user` SET `SoluotTao`= `SoluotTao`-'$Soluot' WHERE `IdUser` ='$IdUser'";
        $result = $db->query_execute($sql);
